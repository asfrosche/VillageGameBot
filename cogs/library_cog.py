...
class GameLibrary(commands.Cog):
    ...
    async def lib_add(self, ctx, role_or_host: str = None, team: int = None, *args):
        """
        Usage:
        ROLE:
        ...
        # HOST MODE
        # ==========================================================
        if role_or_host and role_or_host.lower() == "host":
            mention_pattern = r'<@!?(\d+)>'
        ...
        # ==========================================================
        # ROLE MODE (unchanged behavior)
        # ==========================================================
        if role_or_host is None or team is None:
            await ctx.send("❌ Usage: .lib add <role_name> <team> [@player] [@sponsor]")
            return

        role_name = role_or_host
    ...
    async def lib_help(self, ctx):
        """Display help"""
        embed = discord.Embed(title="📚 Game Library — Help", description="Browse roles, view stats, manage library.", color=EMBED_COLOR)
